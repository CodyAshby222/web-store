...
exports.user__update = (req, res) => {
  //name, email, password
  User.find(
    { key: { $ne: req.query.key }, email: req.body.email },
    (err, user) => {
      if (user.length || err) {
        res.json(false);
        return;
        ...
        if (req.body.password)
          hashString(req.body.password, updateUser, req, res);
  ...
  Cart.find({ userKey: req.query.key }, (err, result) => {
    ...
    if (item) {
      Cart.findOneAndUpdate(
        { userKey: req.query.key, itemID: req.body.itemID },
  ...
  req.body.cart.forEach((item) => {
    if (item.quantity <= 0) {
      Cart.findOneAndDelete(
        { userKey: req.query.key, itemID: item.itemID },
        (err, result) => {
          if (err) {
  ...
  Cart.find({ userKey: req.query.key }, (err, result) => {
    ...
    result.forEach((item) => {
      if (item.itemID && item.quantity) {
        ...
        Cart.findOneAndDelete(
          { _id: item._id, userKey: req.query.key },
          (err, result) => {
            if (err) res.send(err);
          }
        );
